...
void augument(struct state* S, struct state* I) {
    if (I->prod[0][0] == 'S')
        strcpy_s(S->prod[0], sizeof(S->prod[0]), "Z->.S");
    else {
        strcpy_s(S->prod[0], sizeof(S->prod[0]), "S->.");
            ...
            for (int j = 0; j < I->prod_count; j++) {
                if (I->prod[j][0] == a) {
                    if (!in_state(I0, I->prod[j])) {
                        strcpy_s(I0->prod[I0->prod_count], sizeof(I0->prod[I0->prod_count]), I->prod[j]);
                        I0->prod_count++;
                    }
                }
            }
...
void goto_state(struct state* I, struct state* S, char a) {
    ...
    for (int i = 0; i < I->prod_count; i++) {
        if (char_after_dot(I->prod[i]) == a) {
            ...
            strcpy_s(S->prod[S->prod_count], sizeof(S->prod[S->prod_count]), move_dot(I->prod[i], strlen(I->prod[i])));
            S->prod_count++;
        }
    }
}
...
void print_shift_table(int state_count) {
    const int nt_t_count = no_nt + no_t;
    const int acc_index = no_t - 1;
    ...
    for (int i = 0; i < state_count; i++) {
        std::vector<int> arr(nt_t_count, -1);
